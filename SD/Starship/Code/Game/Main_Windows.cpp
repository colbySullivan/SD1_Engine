...
void App_Render()
{
	...
	// ToDo set this to floats values these are incorrect step 9
	glClearColor( 100.f, 50.f, 0.f, 1.f ); // Note; glClearColor takes colors as floats in [0,1], not bytes in [0,255]
...
void App_Run()
{
	...
	while( !g_isQuitting )			// #SD1ToDo: ...becomes:  !g_theApp->IsQuitting()
	{
		...
		App_Update();		// #SD1ToDo: ...becomes just Update();		once this function becomes App::Run()
